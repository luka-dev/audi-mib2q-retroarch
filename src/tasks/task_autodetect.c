   ...
   if (!match_found)
   {
      ...
      if (string_is_equal(autoconfig_handle->device_info.joypad_driver,
         ...
         fallback_device_name = "Gamepad";
            ...
            "qnx"))
         fallback_device_name = "QNX HID Gamepad";
               ...
               fallback_device_name))
      {
         ...
         /* Prefer a replaceable external fallback. This lets constrained
          * platforms ship controller resources beside the binary instead of
          * baking policy into the frontend. Existing platforms still fall
          * back to their built-in definitions when no file is present. */
         fallback_matched = input_autoconfigure_scan_config_files_external(
               autoconfig_handle);
         if (!fallback_matched)
            fallback_matched = input_autoconfigure_scan_config_files_internal(
                  autoconfig_handle);
