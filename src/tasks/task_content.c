      ...
      content_state_t *p_content)
{
   ...
   /* Appliance builds may provide a rule file that maps media folders to
    * playlists and cores. Start discovery only after History/Favorites are
    * initialised, so stale managed entries can be removed from both safely. */
   {
      static bool content_discovery_started = false;
      if (!content_discovery_started && getenv("RA_CONTENT_RULES"))
         content_discovery_started = task_push_content_discovery();
   }
