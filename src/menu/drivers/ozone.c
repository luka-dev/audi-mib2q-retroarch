...
struct ozone_handle
{
   ...
   unsigned wallpaper_width;
   unsigned wallpaper_height;
...
/* Audi MIB2 High stock colour plates.  These values are taken from
 * HMITerminalImplMIB2High.createFallbackScreen() in the MU1316 HMI:
 * white/black plus #9A9A9A, #AAAAAA and #B5B5B5 neutral roles, with
 * the modelled red, amber, blue and green accent pairs.  Keep this as a
 * native Ozone theme: the wallpaper and fonts remain replaceable SD assets. */
static const float ozone_background_libretro_running_audi[16] = {
...
/* A translucent black sidebar preserves the OEM background while retaining
 * enough contrast for Audi Type Display High at the HU's 1024x480 size. */
static const float ozone_sidebar_background_audi[16] =
   COLOR_HEX_TO_FLOAT(0x000000, 0.58f);
static const float ozone_sidebar_gradient_top_audi[16] =
   COLOR_HEX_TO_FLOAT(0x000000, 0.68f);
static const float ozone_sidebar_gradient_bottom_audi[16] =
   COLOR_HEX_TO_FLOAT(0x000000, 0.48f);

static const float ozone_border_0_audi_red[16] =
   COLOR_HEX_TO_FLOAT(0xFF0019, 1.0f);
static const float ozone_border_1_audi_red[16] =
   COLOR_HEX_TO_FLOAT(0xB83A49, 1.0f);
static const float ozone_border_0_audi_neutral[16] =
   COLOR_HEX_TO_FLOAT(0xFFFFFF, 1.0f);
static const float ozone_border_1_audi_neutral[16] =
   COLOR_HEX_TO_FLOAT(0xB5B5B5, 1.0f);
static const float ozone_border_0_audi_amber[16] =
   COLOR_HEX_TO_FLOAT(0xFFB400, 1.0f);
static const float ozone_border_1_audi_amber[16] =
   COLOR_HEX_TO_FLOAT(0xA2916D, 1.0f);
static const float ozone_border_0_audi_blue[16] =
   COLOR_HEX_TO_FLOAT(0x1E8CAA, 1.0f);
static const float ozone_border_1_audi_blue[16] =
   COLOR_HEX_TO_FLOAT(0x6D97A2, 1.0f);
static const float ozone_border_0_audi_green[16] =
   COLOR_HEX_TO_FLOAT(0x64AA00, 1.0f);
static const float ozone_border_1_audi_green[16] =
   COLOR_HEX_TO_FLOAT(0x739B75, 1.0f);

#define OZONE_AUDI_THEME_INIT(primary, primary_rgba, muted, cursor_0, cursor_1) { \
   COLOR_HEX_TO_FLOAT(0x000000, 1.0f),                  /* background */ \
   ozone_background_libretro_running_audi,             /* running background */ \
   COLOR_HEX_TO_FLOAT(0x9A9A9A, 1.0f),                 /* header/footer separator */ \
   COLOR_HEX_TO_FLOAT(0xFFFFFF, 1.0f),                 /* text */ \
   COLOR_HEX_TO_FLOAT(muted, 1.0f),                    /* selection fill */ \
   COLOR_HEX_TO_FLOAT(primary, 1.0f),                  /* selection border */ \
   COLOR_HEX_TO_FLOAT(0x9A9A9A, 1.0f),                 /* entry border */ \
   COLOR_HEX_TO_FLOAT(0xB5B5B5, 1.0f),                 /* entry icon */ \
   COLOR_HEX_TO_FLOAT(primary, 1.0f),                  /* selected icon */ \
   COLOR_HEX_TO_FLOAT(0x000000, 1.0f),                 /* message background */ \
   0xFFFFFFFF,                                          /* text */ \
   0xAAAAAAFF,                                          /* sidebar text */ \
   primary_rgba,                                        /* selected text */ \
   0x9A9A9AFF,                                          /* sublabel */ \
   primary,                                             /* screensaver tint */ \
   ozone_sidebar_background_audi,                       /* sidebar */ \
   ozone_sidebar_gradient_top_audi,                     /* sidebar top */ \
   ozone_sidebar_gradient_bottom_audi,                  /* sidebar bottom */ \
   cursor_0,                                            /* cursor border start */ \
   cursor_1,                                            /* cursor border end */ \
   {0},                                                 /* textures */ \
   "dark"                                              /* reuse stock dark cursor bitmaps */ \
}

static ozone_theme_t ozone_theme_audi_red =
   OZONE_AUDI_THEME_INIT(0xFF0019, 0xFF0019FF, 0xAB5F68,
         ozone_border_0_audi_red, ozone_border_1_audi_red);
static ozone_theme_t ozone_theme_audi_neutral =
   OZONE_AUDI_THEME_INIT(0xFFFFFF, 0xFFFFFFFF, 0x9A9A9A,
         ozone_border_0_audi_neutral, ozone_border_1_audi_neutral);
static ozone_theme_t ozone_theme_audi_amber =
   OZONE_AUDI_THEME_INIT(0xFFB400, 0xFFB400FF, 0xA2916D,
         ozone_border_0_audi_amber, ozone_border_1_audi_amber);
static ozone_theme_t ozone_theme_audi_blue =
   OZONE_AUDI_THEME_INIT(0x1E8CAA, 0x1E8CAAFF, 0x6D97A2,
         ozone_border_0_audi_blue, ozone_border_1_audi_blue);
static ozone_theme_t ozone_theme_audi_green =
   OZONE_AUDI_THEME_INIT(0x64AA00, 0x64AA00FF, 0x739B75,
         ozone_border_0_audi_green, ozone_border_1_audi_green);

#undef OZONE_AUDI_THEME_INIT
...
static ozone_theme_t *ozone_themes[] = {
   ...
   &ozone_theme_audi_red,
   &ozone_theme_audi_neutral,
   &ozone_theme_audi_amber,
   &ozone_theme_audi_blue,
   &ozone_theme_audi_green,
};
      ...
      unsigned color_theme)
{
   ...
   switch (color_theme)
   {
      ...
      case OZONE_COLOR_THEME_BASIC_BLACK:
         ...
         break;
      case OZONE_COLOR_THEME_AUDI_RED:
         theme = &ozone_theme_audi_red;
         break;
      case OZONE_COLOR_THEME_AUDI_NEUTRAL:
         theme = &ozone_theme_audi_neutral;
         break;
      case OZONE_COLOR_THEME_AUDI_AMBER:
         theme = &ozone_theme_audi_amber;
         break;
      case OZONE_COLOR_THEME_AUDI_BLUE:
         theme = &ozone_theme_audi_blue;
         break;
      case OZONE_COLOR_THEME_AUDI_GREEN:
         theme = &ozone_theme_audi_green;
         break;
   ...
   switch (action)
   {
      ...
      case MENU_ACTION_CANCEL:
         ...
         if (     (settings->bools.kiosk_mode_enable
               ...
               && !(ozone->flags2 & OZONE_FLAG2_WANT_FULLSCREEN_THUMBNAILS))
         {
            action_ok_quit(NULL, NULL, 0, 0, 0);
            new_action = MENU_ACTION_NOOP;
            break;
         }
...
static void ozone_context_reset(void *data, bool is_threaded)
{
   ...
   if (ozone)
   {
      ...
      /* Ozone does not support menu wallpapers upstream.  The Audi HU uses
       * the OEM MMI background as an external SD-card resource, so load the
       * standard menu_wallpaper setting here without embedding it in the
       * executable.  Loading is synchronous on QNX, which also keeps the EGL
       * texture upload on the context-owning thread. */
      video_driver_texture_unload(&ozone->wallpaper_texture);
      ozone->wallpaper_width  = 0;
      ozone->wallpaper_height = 0;
      ...
      if (path_is_valid(settings->paths.path_menu_wallpaper))
         gfx_display_reset_icon_texture(
               ...
               &ozone->wallpaper_width,
               &ozone->wallpaper_height);
...
static void ozone_draw_wallpaper(
      gfx_display_t *p_disp,
      void *userdata,
      unsigned video_width,
      unsigned video_height,
      ...
      float opacity)
{
   ...
   const float *default_tex_coord    = (dispctx && dispctx->get_default_tex_coords)
         ? dispctx->get_default_tex_coords() : NULL;
   ...
   /* Preserve the source aspect ratio and crop from the centre as a fallback.
    * The Audi production wallpaper is already pre-cropped to 1024x480, so it
    * takes the equal-aspect path and is drawn pixel-for-pixel at HU size. */
   if (default_tex_coord)
   {
      memcpy(tex_coord, default_tex_coord, sizeof(tex_coord));
      texture_aspect = (float)texture_width / (float)texture_height;
      display_aspect = (float)video_width / (float)video_height;

      if (texture_aspect > display_aspect)
      {
         visible_fraction = display_aspect / texture_aspect;
         crop_offset       = (1.0f - visible_fraction) * 0.5f;
         for (i = 0; i < 4; i++)
            tex_coord[(i * 2)] = crop_offset
                  + (default_tex_coord[(i * 2)] * visible_fraction);
      }
      else if (texture_aspect < display_aspect)
      {
         visible_fraction = texture_aspect / display_aspect;
         crop_offset       = (1.0f - visible_fraction) * 0.5f;
         for (i = 0; i < 4; i++)
            tex_coord[(i * 2) + 1] = crop_offset
                  + (default_tex_coord[(i * 2) + 1] * visible_fraction);
      }

      draw.tex_coord = tex_coord;
   }

   if (dispctx->blend_begin)
      dispctx->blend_begin(userdata);

   gfx_display_draw_bg(p_disp, &draw, &coords, userdata, true, opacity);
   dispctx->draw(&draw, userdata, video_width, video_height);
...
static void ozone_frame(void *data, video_frame_info_t *video_info)
{
    ...
    * all rendering this frame. */
   {
      ...
      wallpaper_tex    = ozone->wallpaper_texture;
      wallpaper_width  = ozone->wallpaper_width;
      wallpaper_height = ozone->wallpaper_height;
   ...
   /* Keep the gameplay/quick-menu video path unchanged.  The external OEM
    * image is the menu background only when no libretro core is running. */
   if (!libretro_running && wallpaper_tex)
      ozone_draw_wallpaper(
            p_disp,
            userdata,
            video_width,
            video_height,
            ...
            wallpaper_width,
            wallpaper_height,
            menu_wallpaper_opacity * ozone->animations.alpha);
