...
/* ra_dbg() is a temporary file-backed tracer used by the QNX bring-up
 * build. Several instrumented common source files are also compiled by the
 * regular desktop build, where the QNX tracer does not exist. Keep those
 * diagnostics QNX-only without requiring platform guards at every call site. */
#if !defined(__QNX__)
#define ra_dbg(...) ((void)0)
#endif
