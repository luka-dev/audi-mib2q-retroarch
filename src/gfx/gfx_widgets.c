...
#if defined(__QNX__)
/* Event-scoped trace remains active after the four-frame runloop trace ends.
 * It is deliberately bounded so a notification failure is visible without
 * turning the display log into an unbounded per-frame stream. */
static unsigned qnx_widget_event_trace_budget = 128;
#define QNX_WIDGET_EVENT_TRACE(...) do { \
      if (qnx_widget_event_trace_budget > 0) { \
         qnx_widget_event_trace_budget--; \
         ra_dbg(__VA_ARGS__); \
      } \
   } while (0)
#else
#define QNX_WIDGET_EVENT_TRACE(...) ((void)0)
#endif
      ...
      if (!msg_widget)
      {
...
#if defined(__QNX__)
         msg_widget->duration                   = qnx_finished_task
               ? TASK_FINISHED_DURATION : duration;
#else
...
#if defined(__QNX__)
         /* A completed task is freed immediately after its main-thread
          * callback. Keeping it attached to a progress/hourglass widget buys
          * nothing and sends QNX through the only render path that still
          * stalls. Present it as a normal styled toast instead: same title,
          * success/error colour and lifetime, no dangling task pointer or
          * progress animation. Running tasks retain the normal task widget. */
         msg_widget->task_ptr                   = qnx_finished_task ? NULL : task;
#else
         msg_widget->task_ptr                   = task;
#endif
         ...
         if (task)
         {
            ...
            if (!qnx_finished_task)
               ...
               QNX_WIDGET_EVENT_TRACE(
                     "widgets qnx completed task -> styled toast: %s",
                     msg_title ? msg_title : "(null)");
#else
            task->frontend_userdata             = msg_widget;
#endif
...
#if defined(HAVE_THREADS) && !defined(__QNX__)
   slock_lock(p_dispwidget->current_msgs_lock);
#endif

   QNX_WIDGET_EVENT_TRACE("widgets qnx move enter size=%u",
         (unsigned)p_dispwidget->current_msgs_size);
...
#endif
   QNX_WIDGET_EVENT_TRACE("widgets qnx move exit");
}
...
#define QNX_WIDGET_TRACE(...) do { \
      if (qnx_runloop_trace_iter < 4) \
         ra_dbg(__VA_ARGS__); \
   } while (0)
#else
#define QNX_WIDGET_TRACE(...) ((void)0)
#endif
   ...
   QNX_WIDGET_TRACE("rl %u widgets iterate enter",
   ...
   QNX_WIDGET_TRACE("rl %u widgets after dpi scale",
         qnx_runloop_trace_iter);
       ...
       (height       != p_dispwidget->last_video_height))
   {
      ...
      QNX_WIDGET_TRACE("rl %u widgets after relayout",
            qnx_runloop_trace_iter);
   }
   ...
   for (i = 0; i < ARRAY_SIZE(widgets); i++)
   {
      ...
      if (widget->iterate)
      {
         QNX_WIDGET_TRACE("rl %u widgets before iterate[%u]",
               qnx_runloop_trace_iter, (unsigned)i);
         widget->iterate(p_dispwidget,
               width, height, fullscreen,
               dir_assets, font_path, is_threaded);
         QNX_WIDGET_TRACE("rl %u widgets after iterate[%u]",
               qnx_runloop_trace_iter, (unsigned)i);
      }
   }
   ...
   {
      disp_widget_msg_t *msg_widget = NULL;
      bool current_msgs_locked      = true;

#ifdef HAVE_THREADS
#if defined(__QNX__)
      QNX_WIDGET_TRACE("rl %u widgets before current-msgs trylock",
            qnx_runloop_trace_iter);
      current_msgs_locked = slock_try_lock(p_dispwidget->current_msgs_lock);
      QNX_WIDGET_TRACE("rl %u widgets current-msgs trylock=%u",
            qnx_runloop_trace_iter, (unsigned)current_msgs_locked);
#else
      slock_lock(p_dispwidget->current_msgs_lock);
#endif
#endif

      if (current_msgs_locked &&
          p_dispwidget->current_msgs_size < ARRAY_SIZE(p_dispwidget->current_msgs))
      {
         bool msg_queue_locked = true;
...
#ifdef HAVE_THREADS
#if defined(__QNX__)
         QNX_WIDGET_TRACE("rl %u widgets before fifo trylock",
               qnx_runloop_trace_iter);
         msg_queue_locked = slock_try_lock(p_dispwidget->msg_queue_lock);
         QNX_WIDGET_TRACE("rl %u widgets fifo trylock=%u",
               qnx_runloop_trace_iter, (unsigned)msg_queue_locked);
#else
         slock_lock(p_dispwidget->msg_queue_lock);
#endif
#endif
         if (msg_queue_locked &&
             FIFO_READ_AVAIL_NONPTR(p_dispwidget->msg_queue) > 0)
         ...
         {
            QNX_WIDGET_EVENT_TRACE(
                  "widgets qnx consume msg task=%u flags=0x%x",
                  (unsigned)(msg_widget->task_ptr != NULL),
                  (unsigned)msg_widget->flags);
      ...
      if (current_msgs_locked && msg_widget)
      {
         QNX_WIDGET_EVENT_TRACE(
               "widgets qnx activate msg task=%u size=%u",
               (unsigned)(msg_widget->task_ptr != NULL),
               (unsigned)p_dispwidget->current_msgs_size);
   ...
   QNX_WIDGET_TRACE("rl %u widgets after queue consume",
         qnx_runloop_trace_iter);
   ...
   QNX_WIDGET_TRACE("rl %u widgets iterate exit",
         qnx_runloop_trace_iter);
#undef QNX_WIDGET_TRACE
}
...
void gfx_widgets_frame(void *data)
{
   ...
   if (p_dispwidget->current_msgs_size)
      QNX_WIDGET_EVENT_TRACE("widgets qnx frame before widget frames size=%u",
            (unsigned)p_dispwidget->current_msgs_size);
   ...
   if (p_dispwidget->current_msgs_size)
      QNX_WIDGET_EVENT_TRACE("widgets qnx frame after widget frames");
   ...
   if (p_dispwidget->current_msgs_size)
   {
      ...
      QNX_WIDGET_EVENT_TRACE("widgets qnx frame message draw enter size=%u",
            (unsigned)p_dispwidget->current_msgs_size);
      ...
      for (i = 0; i < p_dispwidget->current_msgs_size; i++)
      {
         ...
         if (msg->task_ptr)
         {
            QNX_WIDGET_EVENT_TRACE("widgets qnx frame before task msg[%u]",
                  (unsigned)i);
            gfx_widgets_draw_task_msg(
               ...
               msg, userdata,
               video_width, video_height);
            QNX_WIDGET_EVENT_TRACE("widgets qnx frame after task msg[%u]",
                  (unsigned)i);
         }
         else
         {
            QNX_WIDGET_EVENT_TRACE("widgets qnx frame before toast[%u]",
                  (unsigned)i);
            gfx_widgets_draw_regular_msg(
               ...
               msg, userdata,
               video_width, video_height);
            QNX_WIDGET_EVENT_TRACE("widgets qnx frame after toast[%u]",
                  (unsigned)i);
         }
      }
      ...
      slock_unlock(p_dispwidget->current_msgs_lock);
#endif
      QNX_WIDGET_EVENT_TRACE("widgets qnx frame message draw exit");
   }

   /* Ensure all text is flushed */
   if (p_dispwidget->current_msgs_size)
      QNX_WIDGET_EVENT_TRACE("widgets qnx frame before text flush");
   gfx_widgets_flush_text(video_width, video_height,
         &p_dispwidget->gfx_widget_fonts.regular);
   gfx_widgets_flush_text(video_width, video_height,
         &p_dispwidget->gfx_widget_fonts.bold);
   gfx_widgets_flush_text(video_width, video_height,
         &p_dispwidget->gfx_widget_fonts.msg_queue);
   if (p_dispwidget->current_msgs_size)
      QNX_WIDGET_EVENT_TRACE("widgets qnx frame after text flush");
