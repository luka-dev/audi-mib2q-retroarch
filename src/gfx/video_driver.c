...
{
   ra_dbg("!!! video_null_init: RetroArch fell back to the NULL video driver "
          "(no GL context -> nothing on screen)");
   *input      = NULL;
   *input_data = NULL;
...
bool video_driver_init_internal(bool *video_is_threaded, bool verbosity_enabled)
{
   ...
   ra_dbg(">>> video_driver_init_internal: reached, finding driver...");
   video_driver_find_driver(settings, "video driver", verbosity_enabled);
   ra_dbg(">>> picked video driver: ident=%s (about to call ->init)",
         (video_st->current_video && video_st->current_video->ident)
            ? video_st->current_video->ident : "(null)");
