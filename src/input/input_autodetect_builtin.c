...
/* MHI2Q: no QNX Screen client lib / header (Kanzi owns Screen; we go through
 * libdisplayinit). The BB10 screen.h include is not needed for autoconfig. */
...
#ifdef __QNX__
/* Stable physical layout produced by qnx_xusb.c for every XUSB/GIP device.
...
DECL_BTN_EX(y,       2, "X") \
DECL_BTN_EX(x,       3, "Y") \
DECL_BTN_EX(l,       4, "Left Bumper") \
DECL_BTN_EX(r,       5, "Right Bumper") \
DECL_BTN_EX(select,  6, "Back / View") \
DECL_BTN_EX(start,   7, "Start / Menu") \
DECL_BTN_EX(l3,      8, "Left Thumb") \
DECL_BTN_EX(r3,      9, "Right Thumb") \
DECL_BTN_EX(up,   h0up, "D-Pad Up") \
DECL_BTN_EX(down, h0down, "D-Pad Down") \
DECL_BTN_EX(left, h0left, "D-Pad Left") \
DECL_BTN_EX(right, h0right, "D-Pad Right") \
DECL_AXIS_EX(l2,        +2, "Left Trigger") \
DECL_AXIS_EX(r2,        -2, "Right Trigger") \
DECL_AXIS_EX(l_x_plus,  +0, "Left Stick Right") \
DECL_AXIS_EX(l_x_minus, -0, "Left Stick Left") \
DECL_AXIS_EX(l_y_plus,  +1, "Left Stick Down") \
DECL_AXIS_EX(l_y_minus, -1, "Left Stick Up") \
DECL_AXIS_EX(r_x_plus,  +3, "Right Stick Right") \
DECL_AXIS_EX(r_x_minus, -3, "Right Stick Left") \
DECL_AXIS_EX(r_y_plus,  +4, "Right Stick Down") \
DECL_AXIS_EX(r_y_minus, -4, "Right Stick Up") \
DECL_MENU_EX(10, "Guide")
#endif
...
#endif
#ifdef __QNX__
   DECL_AUTOCONF_DEVICE("QNX XInput Controller", "qnx",
         QNX_XINPUT_DEFAULT_BINDS),
#endif
#if defined(DINGUX) && defined(HAVE_SDL_DINGUX)
