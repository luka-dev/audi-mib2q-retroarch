...
{
#if defined(__QNX__)
   extern unsigned qnx_runloop_trace_iter;
#define QNX_RUNLOOP_TRACE(...) do { \
      if (qnx_runloop_trace_iter < 4) \
         ra_dbg(__VA_ARGS__); \
   } while (0)
#else
#define QNX_RUNLOOP_TRACE(...) ((void)0)
#endif
   ...
   QNX_RUNLOOP_TRACE("rl %u check_state enter menu_alive=%u core_running=%u",
         qnx_runloop_trace_iter,
#ifdef HAVE_MENU
         (unsigned)!!(menu_st->flags & MENU_ST_FLAG_ALIVE),
#else
         0u,
#endif
         (unsigned)!!(runloop_st->flags & RUNLOOP_FLAG_CORE_RUNNING));
   ...
   QNX_RUNLOOP_TRACE("rl %u check_state after collect input",
         qnx_runloop_trace_iter);
   ...
   */
   ...
   gfx_animation_update(
         ...
         output_width,
         output_height);
   QNX_RUNLOOP_TRACE("rl %u check_state after animation update",
         qnx_runloop_trace_iter);
   ...
   if (widgets_active)
   {
      ...
      /* gfx_widgets has its own locks for the pending-message FIFO and the
       * displayed-message array.  Holding the legacy runloop message lock
       * around the complete widget pass is therefore unnecessary, and on
       * QNX 6.5 it can deadlock the main thread before menu input is polled.
       *
       * Never let optional notification rendering block input/render. If a
       * producer owns the legacy lock, skip this widget update for one frame;
       * the queued notification remains protected by the widget FIFO lock and
       * will be consumed on a later frame. */
      QNX_RUNLOOP_TRACE("rl %u widgets before outer trylock",
            qnx_runloop_trace_iter);
      if (slock_try_lock(runloop_st->msg_queue_lock))
      {
         QNX_RUNLOOP_TRACE("rl %u widgets outer trylock acquired",
               qnx_runloop_trace_iter);
         gfx_widgets_iterate(
               p_disp,
               settings,
         ...
         slock_unlock(runloop_st->msg_queue_lock);
         QNX_RUNLOOP_TRACE("rl %u widgets complete/unlocked",
               qnx_runloop_trace_iter);
      }
      else
         QNX_RUNLOOP_TRACE("rl %u widgets outer lock busy; skipped frame",
               qnx_runloop_trace_iter);
#else
      RUNLOOP_MSG_QUEUE_LOCK(runloop_st);
      gfx_widgets_iterate(
   ...
   if (menu_st->flags & MENU_ST_FLAG_ALIVE)
   {
      ...
      QNX_RUNLOOP_TRACE("rl %u menu before poll_cb",
            qnx_runloop_trace_iter);
      cbs->poll_cb();
      QNX_RUNLOOP_TRACE("rl %u menu after poll_cb",
            qnx_runloop_trace_iter);
      ...
      QNX_RUNLOOP_TRACE("rl %u menu before menu_event",
            qnx_runloop_trace_iter);
      action                    = (enum menu_action)menu_event(
            settings,
            &current_bits, &trigger_input, display_kb);
      QNX_RUNLOOP_TRACE("rl %u menu after menu_event action=%d",
            qnx_runloop_trace_iter, (int)action);
#ifdef HAVE_NETWORKING
      ...
      {
         bool menu_iterate_ok;
         QNX_RUNLOOP_TRACE("rl %u menu before driver_iterate",
               qnx_runloop_trace_iter);
         menu_iterate_ok = menu_driver_iterate(menu_st, p_disp, anim_get_ptr(),
               settings, action, current_time);
         QNX_RUNLOOP_TRACE("rl %u menu after driver_iterate ok=%u",
               qnx_runloop_trace_iter, (unsigned)menu_iterate_ok);
         if (!menu_iterate_ok)
         {
            if (err_on_init)
            {
      ...
      if (focused || !(runloop_st->flags & RUNLOOP_FLAG_IDLE))
      {
         ...
         if (menu)
         {
            ...
            if (BIT64_GET(menu->state, MENU_STATE_BLIT))
            {
               ...
               {
                  QNX_RUNLOOP_TRACE("rl %u menu before driver render",
                        qnx_runloop_trace_iter);
                  ...
                  QNX_RUNLOOP_TRACE("rl %u menu after driver render",
                        qnx_runloop_trace_iter);
               }
            }
            ...
            {
               bool display_cached;
               QNX_RUNLOOP_TRACE("rl %u menu before display_menu_libretro",
                     qnx_runloop_trace_iter);
               display_cached = display_menu_libretro(runloop_st, input_st,
                        settings->floats.slowmotion_ratio,
                        libretro_running, current_time);
               QNX_RUNLOOP_TRACE("rl %u menu after display_menu_libretro cached=%u",
                     qnx_runloop_trace_iter, (unsigned)display_cached);
               if (display_cached)
               {
                  QNX_RUNLOOP_TRACE("rl %u menu before cached_frame",
                        qnx_runloop_trace_iter);
                  video_driver_cached_frame();
                  QNX_RUNLOOP_TRACE("rl %u menu after cached_frame",
                        qnx_runloop_trace_iter);
               }
            }
            ...
            {
               QNX_RUNLOOP_TRACE("rl %u menu before set_texture",
                     qnx_runloop_trace_iter);
               menu->driver_ctx->set_texture(menu->userdata);
               QNX_RUNLOOP_TRACE("rl %u menu after set_texture",
                     qnx_runloop_trace_iter);
            }
         ...
         {
            QNX_RUNLOOP_TRACE("rl %u menu before audio sample",
                  qnx_runloop_trace_iter);
            audio_driver_menu_sample();
            QNX_RUNLOOP_TRACE("rl %u menu after audio sample",
                  qnx_runloop_trace_iter);
         }
      }

      QNX_RUNLOOP_TRACE("rl %u menu render/audio block complete",
            qnx_runloop_trace_iter);
   ...
   {
      QNX_RUNLOOP_TRACE("rl %u check_state return MENU",
            qnx_runloop_trace_iter);
      return RUNLOOP_STATE_MENU;
   }
#endif
   QNX_RUNLOOP_TRACE("rl %u check_state return ITERATE",
         qnx_runloop_trace_iter);
#undef QNX_RUNLOOP_TRACE
   return RUNLOOP_STATE_ITERATE;
}
 ...
 **/
#if defined(__QNX__)
unsigned qnx_runloop_trace_iter = (unsigned)-1;
#endif
...
int runloop_iterate(void)
{
   ...
   qnx_runloop_trace_iter++;
   if (qnx_runloop_trace_iter < 4)
      ra_dbg("rl %u iterate enter", qnx_runloop_trace_iter);
#endif
#ifdef HAVE_DISCORD
...
#if defined(__QNX__)
   if (qnx_runloop_trace_iter < 4)
      ra_dbg("rl %u before check_state", qnx_runloop_trace_iter);
#endif
...
#if defined(__QNX__)
         if (qnx_runloop_trace_iter < 4)
            ra_dbg("rl %u check_state returned QUIT", qnx_runloop_trace_iter);
#endif
...
#if defined(__QNX__)
         if (qnx_runloop_trace_iter < 4)
            ra_dbg("rl %u check_state returned POLLED_AND_SLEEP", qnx_runloop_trace_iter);
#endif
...
#if defined(__QNX__)
         if (qnx_runloop_trace_iter < 4)
            ra_dbg("rl %u check_state returned PAUSE", qnx_runloop_trace_iter);
#endif
#ifdef HAVE_NETWORKING
         /* FIXME: This is an ugly way to tell Netplay this... */
         netplay_driver_ctl(RARCH_NETPLAY_CTL_PAUSE, NULL);
...
#if defined(__QNX__)
         if (qnx_runloop_trace_iter < 4)
            ra_dbg("rl %u check_state returned MENU", qnx_runloop_trace_iter);
#endif
...
#if defined(__QNX__)
         if (qnx_runloop_trace_iter < 4)
            ra_dbg("rl %u check_state returned ITERATE", qnx_runloop_trace_iter);
#endif
         runloop_st->flags       |= RUNLOOP_FLAG_CORE_RUNNING;
         break;
   }
...
#if defined(__QNX__)
      if (qnx_runloop_trace_iter < 4)
         ra_dbg("rl %u throttle enter min=%lld last=%lld",
               qnx_runloop_trace_iter, (long long)frame_limit_min,
...
#if defined(__QNX__)
            if (qnx_runloop_trace_iter < 4)
               ra_dbg("rl %u throttle return after sleep_ms=%u",
                     qnx_runloop_trace_iter, sleep_ms);
#endif
            return 1;
         }
...
#if defined(__QNX__)
      if (qnx_runloop_trace_iter < 4)
         ra_dbg("rl %u before video_frame_delay", qnx_runloop_trace_iter);
#endif
      video_frame_delay(video_st, settings);
#if defined(__QNX__)
      if (qnx_runloop_trace_iter < 4)
         ra_dbg("rl %u after video_frame_delay", qnx_runloop_trace_iter);
#endif
   }
...
#if defined(__QNX__)
   if (qnx_runloop_trace_iter < 4)
      ra_dbg("rl %u iterate return 0", qnx_runloop_trace_iter);
#endif
   return 0;
}
