...
INSTALLDIR = boot/sys
# The toolchain qcc shim places the SDP's standard library search path ahead of
# our -L entries. A plain -lstartup therefore silently selects the stock QNX
# 6.5 archive (A9-only) instead of the BSP archive below (A15 + 1 GiB fixes),
# and the guest dies at "Unsupported CPUID". Pass the archive as an explicit
# object so its identity is unambiguous.
LIBS = lzo ucl drvr
...
# QNX qcc used bare -M here as a bootstrap link-map switch. The Docker qcc is
# backed by GCC, where bare -M means dependency generation and suppresses the
# link completely. No link map is consumed by this BSP, so omit the option.
LDFLAGS_gcc_qcc =
...
EXTRA_OBJS += $(LIBSTARTUP_ROOT)/$(CPU)/$(LIB_VARIANT)/libstartup.a
